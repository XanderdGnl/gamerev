...
// Haal unieke categorieën op voor de dropdown
$categoryStmt = $pdo->query("SELECT DISTINCT category FROM posts");
$categories = $categoryStmt->fetchAll(PDO::FETCH_COLUMN);

// Haal geselecteerde categorie op uit GET-request
$selectedCategory = $_GET['category'] ?? '';

// Pas de query aan afhankelijk van categorie
if (!empty($selectedCategory)) {
    $stmt = $pdo->prepare("
        SELECT posts.id, posts.post_title, posts.content, posts.category, posts.rating, posts.created_at, posts.likes, users.username
        FROM posts
